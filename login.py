...
def check_404(box, step, display):  # 检测是否404，若404则尝试刷新，尝试三次后报错，需改进成报错但不中断
    count = 0
    while True:
        try:
...
def code_verify(corp, pos, wechat, display):
    ...
    for i in range(5):
        ...
        code = wechat.get_code(corp, display)
...
def input_login_info(corp, pos, display):
    ...
    # 点击文本框并输入纳税人识别号
    logger.info('Inputting ShiBieHao...')
    ...
    pyautogui.hotkey('ctrl', 'a')
    pyautogui.press('del')
    pyautogui.typewrite(corp.shibiehao)
    time.sleep(0.3)
    ...
    pyautogui.hotkey('ctrl', 'a')
    pyautogui.press('del')
    pyautogui.typewrite(corp.shoujihao)
    time.sleep(0.3)
    ...
    pyautogui.hotkey('ctrl', 'a')
    pyautogui.press('del')
    pyautogui.typewrite(corp.password)
    time.sleep(0.3)
...
def finish_code_verify(corp, pos, display):
    for i in range(5):
        display.update(f"已进入验证码环节，准备发送验证码并打开微信联系客户！倒计时：{5 - i}")
        time.sleep(1)
    ...
    # 进行验证码验证
    code_verify(corp, pos, wechat, display)
    ...
    if not check_img('my_to_do', my_to_do_box):
        ...
        if not check_img('my_to_do', my_to_do_box):
            ...
            pyautogui.press('f5')
            time.sleep(5)
            check_404(pos, 'Checking Weather Login Success (Check My To Go).', display)
...
def login(corp, pos, display):
    ...
    logger.info(f'Logging in {corp.name}')
...
if __name__ == "__main__":
    ...
    s_driver = start_browser()

    # 读入Excel中的信息
    logger.info('Loading Excel Info...')
    df = pd.read_excel('./info.xlsx')
    cycle = 1
    s_corp = corpInfo(serial=cycle, name='测试公司',
                      shibiehao=str(df.iloc[cycle - 1, 0]),
                      shoujihao=str(df.iloc[cycle - 1, 1]),
                      password=str(df.iloc[cycle - 1, 2]))
    # 创建显示器
    s_display = StatusDisplay("程序已启动", position=(re_coord(300), re_coord(100)))

    # 执行主函数
    main(s_driver, s_corp, s_display, first=True)
...
# 加载滑块和轨道的模板图片
slider_template = cv2.imread('./img/slider_template.png', 0)
track_template = cv2.imread('./img/track_template.png', 0)

# 使用模板匹配找到滑块和轨道的位置
def find_template_position(image, template):
    result = cv2.matchTemplate(image, template, cv2.TM_CCOEFF_NORMED)
    min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(result)
    return max_loc

slider_position = find_template_position(cv2.cvtColor(screenshot, cv2.COLOR_BGR2GRAY), slider_template)
track_position = find_template_position(cv2.cvtColor(screenshot, cv2.COLOR_BGR2GRAY), track_template)

print(f"滑块位置: {slider_position}")
print(f"轨道位置: {track_position}")

# 计算滑动距离
slider_x, slider_y = slider_position
track_x, track_y = track_position
distance = track_x - slider_x


# 使用 pyautogui 模拟滑动操作
try:
    # 将滑块位置转换为屏幕坐标
    slider_center_x = slider_x + slider_template.shape[1] // 2
    slider_center_y = slider_y + slider_template.shape[0] // 2

    # 移动鼠标到滑块位置
    pyautogui.moveTo(slider_center_x, slider_center_y, duration=0.5)

    # 按住鼠标左键
    pyautogui.mouseDown()

    # 模拟拖动操作
    pyautogui.moveRel(distance, 0, duration=1)  # 水平拖动
